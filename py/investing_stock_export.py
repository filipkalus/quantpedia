...
def main():
    ...
    stocks = investpy.stocks.search_stocks('isin', 'US1718715022')
    if len(stocks) == 1:
        stock = stocks.iloc[0]

        # print(stock)
        # info = investpy.get_stock_information(stock.symbol, stock.country)
        profile = investpy.get_stock_company_profile(stock.symbol, stock.country)
        ...
        req = requests.get(profile_url, headers=head)
        ...
        path_ = root_.xpath("//div[contains(@class, 'companyProfileHeader')]") 
        if path_:
            # company_profile = str(path_[0].text_content())
            industry = path_[0][0][0].text_content()
            sector = path_[0][1][0].text_content()
